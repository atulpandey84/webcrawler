...
from analytics import analyze_social_data_stream
import json
import os
...
class Message(db.Model):
    ...
    is_analytics = db.Column(db.Boolean, default=False)
...
def get_session(session_id):
    ...
    messages = [{'role': m.role, 'content': m.content, 'is_analytics': m.is_analytics} for m in session.messages]
...
def chat():
    ...
    use_analytics = data.get('analytics', False)
    use_dynamic = data.get('dynamic', False)
    ...
    if session_id:
        ...
        # New session
        try:
            content = crawl_website(url, recursive=recursive, use_playwright=use_dynamic)
            if not content:
    ...
    def generate():
        ...
        if use_analytics:
            stream_gen = analyze_social_data_stream(session.content, model)
        else:
            stream_gen = generate_insights_stream(session.content, model, user_prompt, chat_history)
        ...
        # Save history
        user_msg = Message(session_id=session.id, role='user', content=user_prompt or ("Deep Analytics" if use_analytics else ""))
        ai_msg = Message(session_id=session.id, role='assistant', content=full_response, is_analytics=use_analytics)
